...
API_URL = ('https://api.openweathermap.org/data/2.5/onecall?lat={}&lon={}&exclude={}&units=imperial&appid={}')

def query_api(lat, lon, exclude):
    """submit the API query using variables for zip and API_KEY"""
    try:
        # print(API_URL.format(zip, API_KEY))
        data = requests.get(API_URL.format(lat, lon, exclude, API_KEY)).json()
    except Exception as exc:
        print(exc)
        data = None
    return data
    
@app.route('/weather/<lat>:<lon>:<exclude>')
def result(lat, lon, exclude):
    """
    options for exclude:
    ...
    resp = query_api(lat, lon, exclude)
